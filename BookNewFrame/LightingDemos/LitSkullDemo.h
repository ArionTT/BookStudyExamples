...
class LitSkullApp : public DXApp 
{
public:
...
: DXApp(hInstance), mShapesVB(0), mShapesIB(0), mSkullVB(0), mSkullIB(0), mSkullIndexCount(0), mLightCount(1),
...
bool LitSkullApp::Init()
{
	...
	Effects::InitAll(m_pDevice);
	InputLayouts::InitAll(m_pDevice);
...
void LitSkullApp::UpdateScenes(float dt)
{
...
void LitSkullApp::BuildSkullGeometryBuffers()
{
    ...
    D3D11_SUBRESOURCE_DATA vinitData;
    vinitData.pSysMem = &vertices[0];
    HR(m_pDevice->CreateBuffer(&vbd, &vinitData, &mSkullVB));
    ...
    D3D11_SUBRESOURCE_DATA iinitData;
	iinitData.pSysMem = &indices[0];
    HR(m_pDevice->CreateBuffer(&ibd, &iinitData, &mSkullIB));
}
